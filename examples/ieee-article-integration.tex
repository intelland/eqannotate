...
The text following Eq.~\eqref{eq:ieee-update} should resume cleanly below the annotation band. A second display uses manual placement for a label whose desired location is asymmetric.
...
\end{annotatedequation}
Normal prose continues after the manual annotation through the package's reserved vertical space.
...
\end{annotatedequation}
The layout should remain contained within the column and preserve readable text flow. Additional prose fills the remainder of the column to evaluate the equation in page context. Compact article formats make whitespace especially visible, so this fixture is useful for visual regression.
