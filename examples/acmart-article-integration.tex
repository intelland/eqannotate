...
\begin{document}
\begin{abstract}
This fixture checks article-scale spacing, narrow-column wrapping, numbering, and manual placement in an ACM-style two-column paper.
\end{abstract}
\maketitle
...
Text after Eq.~\eqref{eq:acm} remains below the lower annotation band. The next example uses manual placement while preserving the article's normal spacing.
...
\end{annotatedequation}
Normal prose resumes after the manual annotation. This final paragraph checks overlay clearance from surrounding text and verifies the display within article flow.
\section{Conclusion}
