...
term, declare a label, then compile to convergence. It is automatic first,
with a manual fallback for exceptional formulas. No shell escape, Python,
network access, or external service is needed.
...
\end{verbatim}
The optional \texttt{prefer} value is a soft side preference, not a coordinate.
...
\mathcal{L}(\theta)=\eqmark[blue]{reconstruction}{\lambda_1\mathcal{L}_{\rm rec}}
{}+\eqmark[orange]{adversarial}{\lambda_2\mathcal{L}_{\rm adv}}\\
{}+\lambda_3\mathcal{L}_{\rm cyc}
{}+\eqmark[green]{prior}{\lambda_4\mathcal{R}(\theta)}

\eqannotate{reconstruction}{Reconstruction term}
\eqannotate{adversarial}{Adversarial term}
\eqannotate{prior}{Prior regularizer}
\end{annotatedmultline}
The multline wrapper retains multline-style first/last-row alignment. It also
supports the native commands \cmd{\shoveleft} and \cmd{\shoveright}.
...
local \cmd{\linewidth}, and reserves vertical space. These are layout
strategies, not an unconditional promise that every imaginable document layout
can be solved without intervention.
...
\eqannotatemanual[<label TikZ options>][<to-path options>]
  {<id>}{<label>}
\end{verbatim}
Use manual placement only when a formula needs exact control:
\begin{annotatedequation}
...
\end{annotatedequation}
Manual labels retain the theme, callout style, masking, and vertical reservation,
but bypass automatic collision avoidance, column clamping, lane assignment, and
crossing optimization for that label. A manual declaration wins over an
automatic declaration for the same ID and emits a warning.

\section{Numbering and references}
Each numbered wrapper has one outer number. Native per-row numbering for
aligned/gathered wrappers and arbitrary custom \cmd{\tag} support are outside
v0.1. Place \cmd{\label} inside a numbered wrapper and reference it normally.
...
\section{Known limitations and v0.1 boundaries}
EqAnnotate does not annotate inline mathematics. v0.1 has no per-row numbering
for multi-line wrappers, arbitrary \cmd{\tag}, or \cmd{\intertext}/
\cmd{\shortintertext} inside its wrappers. On a non-white page, set:
\begin{verbatim}
\renewcommand\eqannotatebackgroundcolor{<color>}
\end{verbatim}
Automatic page-background detection is not provided.
...
\end{itemize}
They illustrate use of the public interface, not implementation knowledge.
